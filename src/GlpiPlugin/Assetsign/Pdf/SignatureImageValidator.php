...
final class SignatureImageValidator
{
   ...
   public static function assertValid(string $dataUri): void {
      if (!preg_match('/^data:image\/png;base64,(.+)$/', $dataUri, $matches)) {
          throw new \RuntimeException(__('Format de signature invalide.', 'assetsign'));
      }

       $binary = base64_decode($matches[1], true);
      if ($binary === false || $binary === '') {
          throw new \RuntimeException(__('Signature illisible.', 'assetsign'));
      }
      ...
      if ($width < self::MIN_WIDTH || $height < self::MIN_HEIGHT) {
          throw new \RuntimeException(__('Signature trop petite, merci de recommencer.', 'assetsign'));
      }
      ...
      if ($inkPixels < self::MIN_INK_PIXELS || $ratio < self::MIN_INK_RATIO) {
          throw new \RuntimeException(__('Signature vide ou trop peu tracée, merci de signer à nouveau.', 'assetsign'));
      }
   }
