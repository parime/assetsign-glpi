 ...
 * style, etat des lieux visuel) que HandoverPdfBuilder (Assetsign), via le
 * trait partage PdfRenderingHelpers — cf. templates/pdf/maintenance.html.twig.
 *
 * Contrairement a Assetsign (PDF non signe genere a la creation, remplace par un
...
final class MaintenancePdfBuilder
{
   ...
   public function renderHtml(Maintenance $maintenance, ?string $signatureImage = null, ?string $signedAt = null): string {
       ...
       return TemplateRenderer::getInstance()->render('@assetsign/pdf/maintenance.html.twig', [
           ...
           'itemtype'            => Assetsign::getCanonicalItemtypeLabel($maintenance->fields['itemtype']),
   ...
   private function storeAsDocument(Maintenance $maintenance, string $pdfBinary): Document {
       ...
       $tmpPath = GLPI_TMP_DIR . '/' . uniqid('assetsign_maintenance_', true) . '.pdf';
