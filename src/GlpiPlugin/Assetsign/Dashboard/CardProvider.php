...
final class CardProvider
{
   public static function pending(array $params = []): array {
       return self::countByStatus(Assetsign::STATUSES_AWAITING_SIGNATURE, 'En attente de signature', $params);
   }

   public static function signed(array $params = []): array {
       return self::countByStatus([Assetsign::STATUS_SIGNED], 'Signées', $params);
   }

   public static function expired(array $params = []): array {
       return self::countByStatus([Assetsign::STATUS_EXPIRED], 'Expirées', $params);
   }

    /**
     * Echecs de creation automatique (createAssetsign()/launchWorkflow()) des
     ...
     * files/_log/assetsign.log a la main.
     */
   public static function failures(array $params = []): array {
       ...
       return [
           ...
           'url'    => $CFG_GLPI['root_doc'] . '/plugins/assetsign/front/config.php',
   ...
   private static function countByStatus(array $statuses, string $label, array $params): array {
       ...
       return [
           'number' => $count,
           'url'    => $CFG_GLPI['root_doc'] . '/plugins/assetsign/front/assetsign.php',
           'label'  => __($label, 'assetsign'),
           'icon'   => Assetsign::getIcon(),
       ];
   }
}
