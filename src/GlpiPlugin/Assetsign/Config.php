...
class Config extends CommonDBTM
{
   ...
   public static function getTypeName($nb = 0): string {
       return __('AssetSign — Signature électronique', 'assetsign');
   }
   ...
   public static function showConfigForm(int $entities_id): void {
      ...
      if ($effectiveLogoId > 0) {
       ...
       \Glpi\Application\View\TemplateRenderer::getInstance()->display('@assetsign/config_form.html.twig', [
           ...
           // fournisseur qui ferait planter la creation de toute assetsign (RuntimeException non
           // rattrapee auparavant, cf. plugin_assetsign_item_assignment()).
           'providers'       => ['canvas' => __('Signature intégrée (canvas)', 'assetsign')],
   ...
   public static function uploadLogo(array $file, int $entities_id): int {
      ...
      if (($file['error'] ?? null) !== UPLOAD_ERR_OK || empty($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) {
          \Session::addMessageAfterRedirect(__('Échec de l\'envoi du logo.', 'assetsign'), false, ERROR);
          return 0;
      }

       $extension = strtolower((string) pathinfo($file['name'] ?? '', PATHINFO_EXTENSION));
      if (!in_array($extension, self::LOGO_ALLOWED_EXTENSIONS, true) || @getimagesize($file['tmp_name']) === false) {
          \Session::addMessageAfterRedirect(__('Le logo doit être une image (PNG, JPG, GIF ou WEBP).', 'assetsign'), false, ERROR);
          return 0;
      }

       $tmpName = uniqid('assetsign_logo_', true) . '.' . $extension;
     ...
     * une vraie assetsign pour resoudre ses tags ##assetsign.xxx##) : verifie
   ...
   public static function sendTestEmail(int $entities_id): string {
      ...
      if (!$email) {
          return __('Votre compte GLPI n\'a pas d\'adresse e-mail enregistrée.', 'assetsign');
      }
      ...
      try {
         ...
         if ($senderEmail !== '') {
          ...
          $mail->to($email);
          $mail->subject(__('E-mail de test — Assetsign & signature', 'assetsign'));
          $mail->html('<p>' . __('Cet e-mail confirme que la configuration d\'envoi du plugin Assetsign & signature fonctionne.', 'assetsign') . '</p>');

         if (!$mailer->send()) {
             return __('Le serveur de messagerie a refusé l\'envoi.', 'assetsign');
         }
      } catch (\Throwable $e) {
          return $e->getMessage();
      }
   ...
   public static function getLatestGithubVersion(): ?string {
       ...
       $cacheKey = 'plugin_assetsign_latest_github_version';
   ...
   public static function getItemtypeLabels(): array {
     ...
     * logique de declenchement de Assetsign::handleItemAssignment() fonctionne donc
     * sans adaptation.
     */
   public static function getAllManageableItemtypes(): array {
       global $DB;

       $itemtypes = PLUGIN_ASSETSIGN_DEFAULT_ITEMTYPES;

       // Volontairement une requete SQL directe plutot que
       // Glpi\Asset\AssetDefinitionManager::getInstance()->getDefinitions() : cette
       // methode est utilisee depuis plugin_init_assetsign(), qui s'execute (listener
   ...
   public function getManagedItemtypes(): array {
       ...
       return is_array($decoded) && count($decoded) > 0 ? $decoded : PLUGIN_ASSETSIGN_DEFAULT_ITEMTYPES;
   }
   ...
   public function getReminderDelays(): array {
    ...
    /** États GLPI (glpi_states) qui, une fois atteints, déclenchent une assetsign. */
   ...
   public static function install(Migration $migration): void {
      ...
      if (!$DB->tableExists($table)) {
         ...
         if (!$DB->fieldExists($table, 'health_score_good_threshold')) {
          ...
          // la colonne is_default de glpi_plugin_assetsign_templates.
         if ($DB->fieldExists($table, 'default_plugin_assetsign_templates_id')) {
             $migration->dropField($table, 'default_plugin_assetsign_templates_id');
         }
      }
   }
}
