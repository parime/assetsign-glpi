 ...
 * createRemise() echouant a l'insertion en base (rare, ex. probleme DB), soit
 * une exception levee pendant launchWorkflow() (generation PDF, jeton,
 * notification) et interceptee par le garde-fou de hook.php
 * (plugin_remise_item_assignment()) — ces echecs sont deja isoles pour ne
 ...
 * files/_log/remise.log a la main : aucune trace en base, donc aucun moyen de
...
class CreationFailure extends CommonDBTM
{
   ...
   public const RECENT_WINDOW_DAYS = 30;
    ...
    /** @param int|null $remiseType Remise::TYPE_* attendu, ou null si inconnu a ce stade (ex: exception generique interceptee dans hook.php). */
   public static function record(string $itemtype, int $items_id, int $entities_id, ?int $remiseType, string $reason): void {
       global $DB;
   ...
   public static function install(Migration $migration): void {
      ...
      if (!$DB->tableExists($table)) {
          ...
          $DB->doQuery("CREATE TABLE `$table` (
                ...
                `remise_type` tinyint unsigned DEFAULT NULL,
                `reason` text,
