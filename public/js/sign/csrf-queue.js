...
// le serveur rejette la seconde en 403. assetsignQueuedFetch() serialise ces
// appels et ne lit le jeton qu'au moment reel de l'envoi (jamais au clic).
(function () {
    'use strict';

    window.ASSETSIGN_CSRF_QUEUE = window.ASSETSIGN_CSRF_QUEUE || Promise.resolve();
    window.assetsignQueuedFetch = window.assetsignQueuedFetch || function (url, buildOptions) {
        var run = function () {
            return fetch(url, buildOptions()).then(function (res) {
                var rotated = res.headers.get('X-Assetsign-Csrf-Token');
                if (rotated) {
                    window.ASSETSIGN_CSRF_TOKEN = rotated;
                }
                return res;
            });
        };
        var next = window.ASSETSIGN_CSRF_QUEUE.then(run, run);
        window.ASSETSIGN_CSRF_QUEUE = next.catch(function () {});
        return next;
    };
})();
