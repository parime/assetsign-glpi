...
**Gabarits de remise** — un gabarit par type de fiche (Remise/Restitution/Don/Vente), avec son propre texte de conditions générales et son statut par défaut. Le gabarit par défaut de chaque type s'édite directement depuis l'onglet correspondant de Configuration > Remise & signature (voir plus bas) — cette liste reste utile pour gérer plusieurs gabarits par type/entité au-delà du seul gabarit par défaut :
...
L'importance de chaque facteur se règle dans Configuration > Remise & signature > Passeport matériel : les poids sont **relatifs** (pas besoin de les faire sommer à 100 exactement), et mettre un poids à 0 désactive simplement ce facteur. Le score peut aussi être désactivé entièrement.
