...
class AssetsignDelegationTest extends AssetsignTestCase
{
   private function makeEmail(int $usersId, string $email): void {
    ...
    /**
     * `Assetsign::getDelegateCandidates()` backs the plain `<select>` on `sign_page.html.twig`
     * (see its own docblock — that page loads no jQuery, so the `User::dropdown()` AJAX widget the
     * other two delegation dropdowns use never initializes there). Confirms it resolves the same
     * `right => 'all'` + entity scope those two other dropdowns use, sorted for display.
     */
   public function testGetDelegateCandidatesReturnsOnlyUsersWithAccessToTheEntitySortedByLabel(): void {
       $entityId = $this->createTestEntity(0, 'PHPUnit DelegateCandidates');
       $otherEntityId = $this->createTestEntity(0, 'PHPUnit DelegateCandidates Other');

       $zebra = $this->createTestUser('Zeb', 'Ra', ['_entities_id' => $entityId]);
       $alpha = $this->createTestUser('Al', 'Pha', ['_entities_id' => $entityId]);
       $outsider = $this->createTestUser('Out', 'Sider', ['_entities_id' => $otherEntityId]);

       $candidates = Assetsign::getDelegateCandidates($entityId);
       $ids = array_column($candidates, 'id');

       $this->assertContains($zebra, $ids);
       $this->assertContains($alpha, $ids);
       $this->assertNotContains($outsider, $ids, 'A user with no access to this entity must never be a candidate.');

       $alphaIndex = array_search($alpha, $ids, true);
       $zebraIndex = array_search($zebra, $ids, true);
       $this->assertLessThan($zebraIndex, $alphaIndex, 'Candidates must be sorted alphabetically by label.');
   }
