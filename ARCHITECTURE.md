...
- **Trivy** (`.github/workflows/ci.yml`) : scan du dépôt à chaque exécution de la CI, restreint volontairement aux secrets et mauvaises configurations (`--scanners secret,misconfig`) — le scan de vulnérabilités des dépendances est délibérément exclu ici, déjà couvert par `composer audit`/`npm audit` (même source de données), pour ne pas dupliquer un résultat.
