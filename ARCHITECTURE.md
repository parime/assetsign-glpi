...
Couverture volontairement partielle au-delà de cette liste (actions groupées `MassiveAction`, notifications, contrôleurs `front/*.php` eux-mêmes, rendu des autres gabarits Twig hors `remise_tab`/`remise_form`) — à étendre au fil des évolutions. Voir [TROUBLESHOOTING.md](TROUBLESHOOTING.md) pour le piège de cache Twig rencontré en testant.
